...
def simSetUp(inpt):
    ...
    if Sim['Record solution']:
        Sim['uu'] = np.zeros((nmax+1,Ndof,NSim))
    else :
        Sim['uu'] = np.zeros((nmax+1,Ndof,1))
    
    Sim['tt'] = np.arange(0,(nmax+1)*Timestep,Timestep)
    Sim['qoiTot'] = np.zeros((nmax+1,1,NSim))

    # For LE calculation
    try:
        Sim['normPerturb'] = float(inpt['normPerturb'])
    except KeyError:
        Sim['normPerturb'] = 1e-6

    Sim['u'] = np.zeros((Ndof,NSim))
    ...
    try:
        Sim['trueCDFFile']=inpt['True CDF file']
    except KeyError:
        pass
