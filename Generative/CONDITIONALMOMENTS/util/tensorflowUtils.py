...
def diversity_example(index,qoi,n_qoi,data,n_data,mean,std):
    """
    Creates a tf.Example message ready to be written to a file.
    ...
    feature = {
            'index'    : _int64_feature(index),
            'qoi'  :     _bytes_feature(qoi),
            'n_qoi'     : _int64_feature(n_qoi),
            'data'  : _bytes_feature(data),
            'n_data'     : _int64_feature(n_data),
...
def mom1_example(index,n_data,n_qoi,qoi,data):
    """
    Creates a tf.Example message ready to be written to a file.
    For writing files with LR and SF
    """
...
def mom2_example(index,n_data,n_qoi,qoi,data2):
    """
    Creates a tf.Example message ready to be written to a file.
    For writing files with LR and SF
    """
...
def _parse_function(example_proto):
    ...
    data = tf.reshape(data, (1 , n_data,1))
    qoi = tf.io.decode_raw(example['qoi'], tf.float64)
    qoi = tf.reshape(qoi, (1 , n_qoi, 1))
    return data, qoi

# Create a dictionary describing the features.
def _mom_parse_function(example_proto):
    """
    Dictionary for reading LR and HR
    """
    image_feature_description = {
...
def _parse_diversity(example_proto):
    """
    Dictionary for reading LR and HR, and conditional moments
    """
    image_feature_description = {
    ...
    n_data = example['n_data']
    mean = tf.io.decode_raw(example['mean'], tf.float64)
    mean = tf.reshape(mean, (1,n_data, 1))
    std = tf.io.decode_raw(example['std'], tf.float64)
    std = tf.reshape(std, (1, n_data, 1))
    data = tf.io.decode_raw(example['data'], tf.float64)
    data = tf.reshape(data, (1 , n_data, 1))
    qoi = tf.io.decode_raw(example['qoi'], tf.float64)
    qoi = tf.reshape(qoi, (1 , n_qoi, 1))
    return data, qoi, mean, std
